...
function Upload_doc() {
  ...
  const PORT_RAG = import.meta.env.VITE_RAG_PORT;
  ...
  const handleSubmit = async (e) => {
    ...
    if (token) {
      try {
        const response = await axios.post(`http://localhost:${PORT}/RAG_server/compress`, formData);
  ...
  return (
    <div className="min-h-screen bg-[#ECEAE6] flex justify-center items-center">
      ...
      >
        ...
        {/* File Upload */}
        <label htmlFor="file-upload" className="block text-[#386641] font-medium mb-1">
          Upload Document:
      ...
      />
        ...
        <button
          type="submit"
          className={`w-full px-4 py-2 bg-[#A7C957] text-white font-bold rounded-lg cursor-pointer transition-transform duration-300 text-lg ${
            isSubmitting ? "opacity-50 cursor-not-allowed" : "hover:bg-[#6A994E] hover:scale-105"
          }`}
